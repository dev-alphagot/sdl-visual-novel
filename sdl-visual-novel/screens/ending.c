...
static void sc_ending_initialize(void) {
	...
	/*dec_ctx->extradata = fmt_ctx->streams[video_stream_idx]->codecpar->extradata;
	dec_ctx->extradata_size = fmt_ctx->streams[video_stream_idx]->codecpar->extradata_size;*/
...
static void sc_ending_render(void) {
	...
	if (av_read_frame(fmt_ctx, pkt) >= 0) {
		if (pkt->stream_index == video_stream_idx) {
			...
			if (got_picture) {
				sprintf(s8, "%.2fs / %.2fs", Mix_GetMusicPosition(v_music), Mix_MusicDuration(NULL));
				text_content(v_txt, s8);
