...
static void sc_wcoll_initialize(void) {
	...
	wcoll_imgs = calloc(4, 2);
	wcoll_txts = calloc(4, 2);
	wcoll_mnns = calloc(4, 2);
	...
	if (!wcoll_imgs || !wcoll_txts) return;
	...
	wcoll_btm = -image_add("image/ui/wcoll_bottom.png", WINDOW_WIDTH / 2, 173 + 120, 1.0f, 1.0f, H_CENTER, TOP);
}
